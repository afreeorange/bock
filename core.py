...
class Bock():
    ...
    def article_title(self, article_path):
        """Return just the article title without a namespace and extension
        """
        # match = re.match(r'^(.*/)?(.*)$', article_path)
        return os.path.basename(os.path.splitext(article_path)[0])
    ...
    def update_search_index_with(self, entity):
    ...
    def delete_from_index(self, article_path):
        ...
        """
        writer = self.__search_index.writer()
        writer.delete_by_term('title', self.article_title(article_path))
        logger.debug('Removed {}'.format(self.article_title(article_path)))

        writer.commit()

    def populate_search_index(self):
        """Wraps the `update_search_index_with` function for the entire
        list of articles
        """
        self.update_search_index_with(self.list_of_articles)
